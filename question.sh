...
#
sed 's/\/20/\//g' data.csv | sed -E 's/^([0-9])\//0\1\//;s/^([0-9]{2})\/([0-9])\//\1\/0\2\//;' | sed 's/^\([0-9][0-9]\)\/\([0-9][0-9]\)\/\([0-9][0-9]\)/20\3-\2-\1/g' | sed 's/;/,/g; s/n/N/g' | sed 's/,/./g' | sed -E 's/,N/,\\N/g' | sed 's/,,/,\\N,/g; s/,\s*$/,\\N/g; s/a/A/g; s/c/C/g
